	...
	shared_ptr<T> TryPop()
	{
		...
		while (true)
		{
			...
			if (_head.compare_exchange_strong(oldHead, ptr->next))
			{
				...
				if (ptr->intenalCount.fetch_add(countIncrease) == -countIncrease)
					delete ptr;

				return res;
			}
			else if (ptr->intenalCount.fetch_sub(1) == 1)
			{
